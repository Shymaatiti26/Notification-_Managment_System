...
const UserSettings = () => {
  const {
    ...
    setIsGroupAdmin,
    groupSenders,
    setGroupSenders,muteUser,setMuteUser,
  } = useAuthContext();
...
//   };
  ...
  const handleMuteGroupChange = async () => {
    ...
    setMuteUser(prevMuteGroup => {
      // Toggle the muteGroup state
      const newMuteGroup = !prevMuteGroup;
  
      // Now that the state has been updated, make the axios.post call
      axios.post(
        "http://localhost:3001/api/v1/setMuteUser",
        { adminId, userId, muteUser: newMuteGroup } // Use the updated state here
      ).then(response => {
        // Handle response if needed
      }).catch(error => {
        // Handle error if needed
